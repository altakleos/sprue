...
> **KB operators:** If you are operating a knowledge base built with Sprue, read [sprue/engine.md](sprue/engine.md) instead. This file is NOT for you.
...
| Operating a KB built with Sprue | [sprue/engine.md](sprue/engine.md) |
| Learning what Sprue does | [sprue/README.md](sprue/README.md) |
...
| Protocols | `sprue/protocols/` | Change how the LLM executes an operation |
| Config + Validators | `sprue/defaults.yaml` + `sprue/scripts/` | Tune a threshold or add a check |
...
```bash
bash sprue/verify.sh                                        # run all validators
bash sprue/verify.sh --file <path>                          # validate one file
...
- **Then:** Update affected protocols in `sprue/protocols/` and validators in `sprue/scripts/`
- **After:** `bash sprue/verify.sh`
...
- **Start at:** `docs/decisions/` — create `0034-<slug>.md` (next number: **0034**)
...
- **Start at:** The protocol in `sprue/protocols/` — read it and the spec it serves
...
- **Start at:** `sprue/defaults.yaml` — find or add the tunable with a comment
- **Then:** Update protocols that reference the dotpath, and wire the key into any validator or script that should consume it (see `sprue/scripts/config.py` for the loader). Search for existing consumers: `grep -r "<dotpath>" sprue/protocols/ sprue/scripts/`
- **After:** `bash sprue/verify.sh`
- **See:** [sprue/defaults.yaml](sprue/defaults.yaml)
...
- **Start at:** `sprue/scripts/` — create `check-<name>.py` or fix the existing script
- **Then:** Register it as a rule in `memory/rules.yaml` (the orchestrator `sprue/scripts/verify.py` loads rules from there — see existing entries for format). NEVER remove comments or logging from scripts.
- **After:** `bash sprue/verify.sh`
...
```bash
bash sprue/verify.sh
```

### Individual Validators

```bash
python3 sprue/scripts/check-frontmatter.py      # YAML frontmatter fields
python3 sprue/scripts/check-tags.py              # facet tag cardinality
python3 sprue/scripts/check-entity-types.py      # entity type registry
python3 sprue/scripts/check-config.py            # config consistency
python3 sprue/scripts/check-fences.py            # code fence validation
python3 sprue/scripts/check-wikilinks.py         # wikilink integrity
python3 sprue/scripts/check-constraints.py       # memory constraint probes
python3 sprue/scripts/check-placement.py         # directory placement rules
python3 sprue/scripts/lint-rules.py              # rules.yaml validation
python3 sprue/scripts/verify.py                  # all rules from memory/rules.yaml
```

### Reset

```bash
bash sprue/reset.sh --level soft|standard|hard              # dry-run (shows what would be deleted)
bash sprue/reset.sh --level soft|standard|hard --confirm    # execute (requires clean git tree)
```
...
docs/
  ...
  decisions/      (32 ADRs)        ← decision records (immutable, next: 0033)
  plans/          (template only)  ← feature task breakdowns
sprue/                             ← ships to users (domain-agnostic engine)
  ...
  scripts/        (20 files)       ← Python validators and utilities
...
- `sprue/` = platform engine. Ships to users. MUST be domain-agnostic.
...
- **Config over hardcoding.** Every tunable value goes in `sprue/defaults.yaml`, referenced via `config.dotpath`.
- **Validators close the loop.** Every invariant from a spec MUST have a mechanical check in `sprue/scripts/`.
