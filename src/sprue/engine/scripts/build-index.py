...
Generates:
  ...
  wiki/.index/by-source-url.yaml — source URL → [{page, claims}]
...
def build_by_source_url(manifest):
    """Reverse index: source URL → [{page, claims}].

    Collects URLs from two places:
    - Page frontmatter sources[].url fields
    - Verification ledger claim entries source_url fields
    """
    # url → {page → set of claim IDs}
    accum: dict[str, dict[str, set]] = defaultdict(lambda: defaultdict(set))
    ...
    for slug, entry in manifest.items():
        ...
        for src in entry.get("sources") or []:
            ...
            if url:
                accum[url][slug]  # register page even without claim IDs
    ...
    if VERIFICATIONS_PATH.exists():
        ...
        for entry in ledger:
            ...
            for claim in entry.get("claims") or []:
                ...
                cid = claim.get("id")
                if url and cid:
                    accum[url][page].add(cid)

    # Flatten to output format
    result = {}
    for url in sorted(accum):
        pages = []
        for page in sorted(accum[url]):
            claims = sorted(accum[url][page])
            pages.append({"page": page, "claims": claims} if claims else {"page": page, "claims": []})
        result[url] = pages
    return result
...
def main():
    ...
    by_source_url = build_by_source_url(manifest)
    write_yaml(
        by_source_url,
        INDEX_DIR / "by-source-url.yaml",
        "# Reverse index: source URL → [{page, claims}]\n# Auto-generated by .sprue/scripts/build-index.py — do not edit\n\n"
    )
    ...
    print(f"   by-source-url:   {len(by_source_url)} source URLs indexed")
