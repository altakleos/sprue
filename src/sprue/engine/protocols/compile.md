...
**Pre-flight checklist** (confirm before writing any page):
- ☐ Images triaged? If the source's `imports.yaml` entry has an `assets` list, Step 4 (Image triage) MUST have run before Step 8 (page generation). Annotations in `instance/state/image-annotations.yaml` are the inputs to image placement and cite-then-claim.
- ☐ Prompt template loaded (Step 5)?
- ☐ Manifest read for cross-linking context (Step 6)?
...
#### Step 4: Triage images

When the source has embedded images, classify and annotate each one before page generation. Image annotations are the inputs to placement decisions and cite-then-claim for image-derived claims — without this step, images are ignored and the wiki page will not include them.

**Gate check:** If `config.images.enabled` is `false` OR the raw source's entry in `instance/state/imports.yaml` has no `assets` list (or it is empty), skip to Step 5.
...
After all assets are annotated, continue to Step 5. The page-writing prompt now has access to the full annotation set for image placement and cite-then-claim attribution of image-sourced claims.

5. Read `.sprue/prompts/<strategy>.md` for the compilation prompt template (or use default wiki_page strategy)
6. Read the wiki manifest for context (existing pages, topic values, domain values)
7. **Assign facets** — read `.sprue/defaults.yaml` → `facets:` for the list of facets, their descriptions, and guardrails. For each facet:
...
8. **Place the page in a wiki directory:**
...
9. Generate the wiki page following .sprue/engine.md conventions:
...
For `provenance: sourced` pages with available raw files, apply the cite-then-claim generation pattern during step 9 page writing. For each verifiable factual claim (version numbers, defaults, limits, behavioral assertions — same extraction criteria as `.sprue/protocols/verify.md` Step 3c):
...
11. Run `bash .sprue/verify.sh --file <path>`. Fix violations before proceeding. (verify auto-normalizes legacy image paths — `raw/assets/…` and `../raw/assets/…` — to the canonical `assets/…` form via `fix-image-paths.py` as a pre-flight.)
